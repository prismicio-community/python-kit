...
from .test_prismic_fixtures import fixture_api, fixture_search, fixture_groups, \
    ...
    fixture_spans_labels, fixture_block_labels, fixture_custom_html, fixture_slices, fixture_composite_slices, fixture_vtiger
import time
import json
import logging
import prismic
...
class PrismicTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.fixture_vtiger = json.loads(fixture_vtiger)
...
class TestFragmentsTestCase(PrismicTestCase):
    ...
    def test_vtiger(self):
        self.maxDiff = 1000
        doc = prismic.Document(self.fixture_vtiger)
        slices = doc.get_slice_zone("canvas.body")
        html = slices.as_html(PrismicTestCase.link_resolver)
        print(html)
